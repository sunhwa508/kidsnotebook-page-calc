...
function measureCodePointWidth(code, fontSize) {
...
function measureCharWidth(char, fontSize) {
    ...
    if (codePoints.includes(0x200d) ||
        codePoints.every((code) => (code >= 0x1f1e6 && code <= 0x1f1ff) || isZeroWidthCodePoint(code))) {
...
function measureTextWidth(text, fontSize) {
    ...
    const flush = () => {
        if (asciiRunLength > 1) {
            const factor = asciiRunLength >= ASCII_LONG_THRESHOLD
