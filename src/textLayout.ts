...
const ASCII_WIDTH_RATIOS: Record<string, number> = {
...
function getCharWidthRatio(char: string): number {
  ...
  if (asciiRatio !== undefined) return asciiRatio;

  const code = char.charCodeAt(0);

  // 한글 음절 (가-힣)
  if (code >= 0xAC00 && code <= 0xD7AF) return KOREAN_SYLLABLE_RATIO;

  // 히라가나 (U+3040-U+309F), 카타카나 (U+30A0-U+30FF)
  if (code >= 0x3040 && code <= 0x30FF) return KANA_RATIO;
  ...
  // 한글 자모 (ㄱ-ㅎ, ㅏ-ㅣ)
  if (code >= 0x3131 && code <= 0x318E) return KOREAN_SYLLABLE_RATIO;

  // 전각 영숫자/기호 (U+FF01-U+FF5E)
  if (code >= 0xFF01 && code <= 0xFF5E) return FULLWIDTH_FALLBACK_RATIO;

  // 이모지 (서로게이트 페어 또는 길이 > 1)
  if (char.length > 1 || code > 0xFFFF) return EMOJI_RATIO;

  // 기타 넓은 문자
  if (code > 127) return FULLWIDTH_FALLBACK_RATIO;
